...
//        DLog.trace( "MasterViewController, viewDidLoad, managedObjectContext: \(managedObjectContext)" )
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if height > bleSize + 20.0 {
            for _ in 0...3 {    // Add ping views
                ...
                let nextColor = UIColor( red: CGFloat(BLEView.randomNumber(Range(uncheckedBounds: (lower: 128, upper: 255))))/CGFloat( 256.0), green: CGFloat(BLEView.randomNumber(Range(uncheckedBounds: (lower: 0, upper: 127))))/CGFloat( 256.0), blue: CGFloat(BLEView.randomNumber(Range(uncheckedBounds: (lower: 0, upper: 127))))/CGFloat( 256.0), alpha: CGFloat( 1.0 ) )
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showCentralDetail" {
            ...
            if segue.identifier == "showPeripheralDetail" {
                DLog.debug( "showPeripheralDetail" )
